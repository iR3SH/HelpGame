	  ...
	  if(!this.stop&&this.fighter.canPlay()&&this.count>0)
	    {
	      ...
	      // Détecte allier à PO max +1 +3 cases (Sur officiel se déplace de 3 cases pour booster son invocateur)
	      if(C!=null&&C.isHide())
	        C=null;
	      ...
	      if(this.fighter.getCurPm(this.fight)>0&&C==null&&this.attack==0)
	      {
	    	...
	    	// On essaie d'aller en face pour flèche ralentissante
	      ...
	      if(this.fighter.getCurPa(this.fight)>0&&C!=null&&!action)
	      {
	    	 ...
	    	 // int celltf=fighter.getCell().getId();
	          ...
	          // On évite de se déplacer inutilement en ligne si on y est déjà
	          if(isEnFace==0)
	          {
	      ...
	      if(this.fighter.getCurPm(this.fight)>0&&!action&&this.attack>0)
	      {
	    	if(movedFar==0)
	    	{
	    		...
	    		movedFar++; // évite qu'il s'éloigne plusieurs fois de suite
	    		if(value!=0)
	    			time=value;
	    	}
	      }
