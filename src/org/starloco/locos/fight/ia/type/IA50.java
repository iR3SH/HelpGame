...
public class IA50 extends AbstractNeedSpell {
  ...
  public void apply() {
    if (!this.stop && this.fighter.canPlay() && this.count > 0) {
      ...
      List<SortStats> invocTonneau = new ArrayList<>();
      invocTonneau.add(allSpells.get(939));
      ...
      if (this.fighter.getCurPa(this.fight) > 0 && !action) {
        if (Function.getInstance().invocIfPossible(this.fight, this.fighter, invocTonneau)) {
          time = 2000;
          action = true;
        }
      }
      ...
      if (this.fighter.getCurPa(this.fight) > 0 && !action) {
        if (Function.getInstance().invocIfPossibleloin(this.fight, this.fighter, invocTonneau)) {
          time = 2000;
          action = true;
        }
      }
