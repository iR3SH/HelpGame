       ...
       // System.out.println("La :" + objectFm.getTemplate().getId() + " " + runeOrPotion + " " + SM );
        ...
        if(runeOrPotion.getTemplate().getId() == 17200){
        	...
        	*/
	        ...
	        if (lvlElementRune > 0 && lvlQuaStatsRune == 0) { // Le cas des runes élémentaire est unique, Le level du mêtier défini la réussite
	            ...
	            */
	            ...
	            // Bon si les poid sont négatifs, on les positionne a 0 pour éviter les bugs, meme si faudrait gèrer le cas différemment
	            if (PoidMaxItem < 0) {
	            	PoidMaxItem = 0;
	            }
    ...
    */
    ...
    public static int currentTotalWeigthBase(String statsModelo, GameObject obj) { // On récupère le poid total de l'item actuel
        ...
        for (String s : split) { // On boucle sur toutes les stats de l'item de base
            ...
            double coef = getPwrPerEffet(statID);   // On récupère le poid de la stat a l'unité
            Weigth = (int) Math.floor(qua * coef); // On multiplie par le jet
            Alto += Weigth;
        } 
        return Alto;  
